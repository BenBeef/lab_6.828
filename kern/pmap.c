...
page_init(void)
{
    ...
    for (i = 1; i < npages; i++) {
        if (i < npages_basemem) {
            ...
            // 4- 我的设想是, 目前使用到了pages结构的末尾，另外还需要去掉内核栈
            pages[i].pp_ref = 1;
        } else {
            pages[i].pp_ref = 0;
            pages[i].pp_link = page_free_list;
...
{
	// Fill this function in
    // pgdir_walk 的本质就是通过va拿到物理地址, 其实就是二级页表翻译的类似版本了
    pde_t *pde;
    pte_t *pgtab;
    struct PageInfo *pp;
...
//        memset(pgtab, 0, PGSIZE);
        ...
        pp->pp_ref += 1;
        pgtab = (pte_t*)KADDR(PTE_ADDR(page2pa(pp)));
...
{
	// Fill this function in
    uintptr_t cnt = size / PGSIZE;
    for (int i=0; i < cnt; i++) {
        if (PGNUM(pa) >= npages) {
            // 超过物理内存的
            pgdir[PDX(va)] = pgdir[PDX(va)] | PTE_P | perm;
        } else {
            page_insert(pgdir, pa2page(pa), (void *)va, perm);
            pa2page(pa)->pp_ref -= 1;
        }
        pa += PGSIZE;
        va += PGSIZE;
...
// the info of page_free_list
void
_info_free_page_list(const char *file, int line, int num)
{
    int len;
    struct PageInfo *pp;
    len = 0;
...
// env
void
_info_env_list(const char *file, int line, void *e)
{
    int pos=0;
    ...
    cprintf("%s:%d idx %d, env.addr %x, env.env_tf %x, tf_eip %x \n", file, line, pos, e,
            ((struct Env*)e)->env_tf, ((struct Env*)e)->env_tf.tf_eip);
    cprintf("+++++++++++++++++++++ end +++++++++++++++++++++ \n\n");
}
...
// the info of page_free_list
void
_fprintf(const char *file, int line, const char *fmt, ...)
{
...
// print memory
void
_printf_mem(const char *file, int line, void *start, int cnt)
{
...
// print memory
void
printf_from_page(void *pgdir, void *va, int cnt)
{
    pte_t *pgtable;
    uint32_t *offset=0, *base;
    ...
    for (int i=0; i<cnt; i++){
        offset = (uint32_t * )((uint32_t)base + PGOFF(va)) + i;
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
    ...
    if (pte_store != NULL) {
        *(pte_store + 1) = pte; // TODO: 这里肯定有error
    }
