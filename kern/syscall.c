...
// transmit a packet
static int
sys_transmit_a_packet(void *srcva, uint16_t len)
{
    user_mem_assert(curenv, srcva, len, PTE_P);
    return transmit_a_packet(srcva, len);
}

//	receive a packet
static int
sys_receive_a_packet(void *dstva, uint32_t *len)
{
    if (dstva == NULL || len == NULL) {
        return -E_INVAL;
    }
    user_mem_assert(curenv, dstva, TX_BLOCK_LEN, PTE_P);
    user_mem_assert(curenv, dstva, 4, PTE_P);
    return receive_a_packet(dstva, len);
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
        ...
        case SYS_transmit_a_packet:
            return sys_transmit_a_packet((void *)a1, (uint16_t)a2);
        case SYS_receive_a_packet:
            return sys_receive_a_packet((void *)a1, (uint32_t *)a2);
        default:
		return -E_INVAL;
	}
}
